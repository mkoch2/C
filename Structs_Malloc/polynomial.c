...
{
    assert(coeff != 0);
    term_t *term_p = malloc(sizeof(term_t));
    assert(term_p != NULL);
    term_p->coeff = coeff;
    term_p->exp = exp;
    return term_p;
...
{
    polynomial_t *polynomial_t_p = malloc(sizeof(polynomial_t));
    assert(polynomial_t_p != NULL);
    polynomial_t_p->num_terms = 0;
    return polynomial_t_p;
}
...
double eval_polynomial(polynomial_t *poly, double x)
{
    ...
    {
        // total += (poly->terms[i]->coeff) * pow(x, poly->terms[i]->exp);
        double evaluate_term = eval_term(poly->terms[i], x);
        total += evaluate_term;
    }
    return total;
}
