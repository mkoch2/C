...
fraction_t reduce(fraction_t f)
{
    ...
    else
    {
        if (f.den < 0)
        {
            f = (fraction_t){-1 * (f.num / g_factor), abs(f.den / g_factor)};
        }
        else
        {
            f = (fraction_t){f.num / g_factor, abs(f.den / g_factor)};
        }
    }
...
fraction_t add_fractions(fraction_t f1, fraction_t f2)
{
    ...
    return make_fraction(add_num, common_den);
}
...
fraction_t multiply_fractions(fraction_t f1, fraction_t f2)
{
    ...
    int common_den = f1.den * f2.den;
    int product_num = f1.num * f2.num;

    return make_fraction(product_num, common_den);
}
