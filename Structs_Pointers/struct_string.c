...
{
   int g_factor = gcd(pf->num, pf->den);
   ...
   else
   {
      if (pf->den < 0)
      {
         *pf = (fraction_t){-1 * (pf->num / g_factor), abs(pf->den / g_factor)};
      }
      else
      {
         *pf = (fraction_t){pf->num / g_factor, abs(pf->den / g_factor)};
      }
   }
}
...
{
   int common_den = pf1->den * pf2->den;

   int add_num = (pf1->num * pf2->den + pf2->num * pf1->den);

   make_fraction(add_num, common_den, psum);
   reduce(psum);
}
...
{
   int common_den = pf1->den * pf2->den;
   int product_num = pf1->num * pf2->num;
   make_fraction(product_num, common_den, pprod);
   reduce(pprod);
}
...
{
   studentp->gpa = calc_gpa(studentp->grades, 6);
}
...
float calc_gpa(const grade_t *gradesp, int numGrades)
{
   ...
   {
      total_gp += gradesp->gp;
      total_weight += gradesp->weight;

      gradesp += 1;
   }

   return total_gp / total_weight;
}
